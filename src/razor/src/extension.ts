...
export async function activate(
	...
	try {
		const razorOptions: RazorLanguageServerOptions =
			resolveRazorLanguageServerOptions(
				vscodeType,
				languageServerDir,
		...
		const languageServiceClient = new RazorLanguageServiceClient(
		...
		const razorCodeActionRunner = new RazorCodeActionRunner(
			languageServerClient,
			logger,
		);
		...
		languageServerClient.onStart(async () => {
			const provisionalCompletionOrchestrator =
				new ProvisionalCompletionOrchestrator(
					documentManager,
			...
			const inlayHintResolveHandler = new InlayHintResolveHandler(
				languageServerClient,
				documentManager,
				logger,
			);
		...
		context.subscriptions.push(
			vscodeType.debug.registerDebugConfigurationProvider(
				"blazorwasm",
				provider,
			),
		);
...
async function startLanguageServer(
	...
	const razorFiles = await vscodeType.workspace.findFiles(
		RazorLanguage.globbingPattern,
	);
	if (razorFiles.length === 0) {
		// No Razor files in workspace, language server should stay off until one is added or opened.
		logger.logAlways(
			"No Razor files detected in workspace, delaying language server start.",
		);

		const watcher = vscodeType.workspace.createFileSystemWatcher(
			RazorLanguage.globbingPattern,
		);
		...
		const razorFileCreatedRegistration = watcher.onDidCreate(async () =>
			delayedLanguageServerStart(),
		);
		const razorFileOpenedRegistration =
			vscodeType.workspace.onDidOpenTextDocument(async (event) => {
				if (event.languageId === RazorLanguage.id) {
					await delayedLanguageServerStart();
				}
			});
		context.subscriptions.push(
			razorFileCreatedRegistration,
			razorFileOpenedRegistration,
		);
	} else {
		await languageServerClient.start();
	}
...
async function setupDevKitEnvironment(
	...
	try {
		...
		logger.logError(
			"Failed to setup DevKit environment for telemetry.",
			error as Error,
		);
	}
}
